...
- **Persisted graph integrity gate**: Added a fail-closed verification gate that ties repository health and incremental reuse to a deterministic digest of the active LadybugDB `Symbol` universe.
  - **Deterministic digest construction**: Full and incremental indexing derive expectations from authoritative in-memory rows before persistence, including file-backed symbols, provider externals, and unresolved dependency placeholders. Verification then streams the complete final active `Symbol` universe in stable UTF-8 keyset order after shadow activation and semantic refresh.
  ...
  - **State, health, and recovery**: `DerivedState` records `unknown`, `verifying`, `verified`, or `failed`. `sdl.repo.status` withholds health until the latest version is verified and directs migrated or failed repositories to a full refresh, followed by a stopped-server clean `.lbug` rebuild if full verification also fails; its compact model projection omits the absolute repository root and last-indexed timestamp.
  - **Concurrent verification safety**: Verified-state and verifier-owned failure publication use compare-and-set updates, so concurrent invalidation or a newer verification result wins.
  - **Durable mutation coverage**: Live-index patches reuse incremental verification to advance the current version's digest after successful watcher, checkpoint, indexed file-write, search-edit, and symbol-edit writes. Patches without a verified baseline invalidate fail-closed, while sync import and pull invalidate before their first graph write so same-version artifact mutations cannot leave stale health marked verified.
