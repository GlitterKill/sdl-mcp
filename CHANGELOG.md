...
- **Provider-first fallback pass-1 stability**: Same-run provider-first legacy fallback remains on inline TypeScript parsing and direct per-file LadybugDB writes while native Rust parsing, parser workers, and the background pass-1 drain accumulator stay disabled for this mixed provider/fallback path. The duplicate-key-safe fresh-copy accumulator mode remains available for disposable-DB validation, but live provider-first fallback does not use it until large LLVM-shaped fallback drains are proven not to hard-exit the process.
