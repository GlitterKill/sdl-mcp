...
- **Provider-first pass-2 diagnostics**: Provider-first fallback diagnostics now include pass-2 target-selection, import-cache, resolver-dispatch, write-active, and write-queue buckets, making large-repo fallback runs distinguish resolver CPU/read work from LadybugDB write contention without enabling broad index diagnostics.
...
- **Provider-first pass-2 writes**: Full-mode pass-2 now splits resolved call edges from unresolved placeholder edges. Large clean resolved-edge batches use the known-symbol `DEPENDS_ON` COPY path after source-symbol replacement, while small batches, unresolved `unresolved:*` targets, and rows with copied edge properties that require CSV quoting keep the generic writer with full-mode skip flags so COPY setup overhead does not dominate small fallback runs and rare C++ provenance with commas, quotes, or newlines cannot break the relationship CSV load; incremental pass-2 remains on the refresh-capable generic writer.
- **Provider-first fallback pass-1 stability**: Same-run provider-first legacy fallback remains on inline TypeScript parsing and direct per-file LadybugDB writes while native Rust parsing, parser workers, and the background pass-1 drain accumulator stay disabled for this mixed provider/fallback path. The inactive fresh-copy accumulator mode is retained for future disposable-DB validation, but it is not used by provider-first fallback until large relationship `COPY` drains are proven stable on LLVM-shaped data.
- **Provider-first shadow handoff**: Full provider-first runs now skip expensive shadow DB staging and finalization when call-proof gaps have already made graph-derived state dirty and activation impossible. The active graph still finalizes normally, and CLI output reports the skipped shadow reason instead of spending time building a shadow DB from untrusted call edges. Runs whose only readiness blocker is the `providerFirst.maxLegacyFallbackFiles` cap still stage and load the provider-primary shadow for diagnostics, while shadow finalization and activation remain skipped because the graph is intentionally partial.
- **Provider-first fallback caps**: Same-run legacy fallback after provider-first materialization is now capped by `indexing.providerFirst.maxLegacyFallbackFiles` (default `5000`). Repos with broad uncovered or provider-unusable tails keep the provider-primary graph, skip the expensive fallback parse, report the skipped file count in coverage output, and leave graph-derived readiness deferred instead of turning a partial provider artifact into a huge tree-sitter fallback run.
