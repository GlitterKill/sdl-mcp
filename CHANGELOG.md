...
- **Tool renaming**: `sdl.agent.orchestrate` → `sdl.agent.context` and `sdl.chain` → `sdl.workflow`. CLAUDE.md/AGENTS.md files generated by prior `init` runs should be regenerated via `sdl-mcp init`.
...
- `sdl.agent.context` MCP tool for policy-aware rung selection with evidence capture
