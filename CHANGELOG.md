...
- **Persisted graph integrity gate**: Full and incremental indexing now derive deterministic symbol digests from authoritative in-memory rows before persistence, including file-backed symbols, provider externals, and unresolved dependency placeholders, then stream the complete final active LadybugDB `Symbol` universe in stable UTF-8 keyset order after shadow activation and semantic refresh. Incremental plans use aggregate repository-owned fileless liveness counts, affected-file buckets, and exact current pass-2 source deltas to stay aligned through symbol promotion, import re-resolution, call cleanup, and placeholder pruning without retaining a second edge graph or trusting cleanup-query output. Repository membership follows `SYMBOL_IN_REPO`; cleanup removes only stale target-repository membership from a shared placeholder, preserves other repositories' relationships and the physical node, and deletes that node only when it has no memberships and is globally isolated. The LadybugDB delete-safety calculation conservatively includes symbols outside or shared with the target repository. Provider expectations start only at the active graph commit boundary, and auto mode no longer falls back after that boundary can leave partial writes. Empty and unsupported files publish their empty expectation before persistence. Unchanged incremental scans verify the existing digest and reuse the latest version instead of manufacturing a snapshot. `DerivedState` records `unknown`, `verifying`, `verified`, or `failed`; `sdl.repo.status` withholds health until the digest is verified for the latest version and directs migrated or failed repositories to a full refresh, followed by a stopped-server clean `.lbug` rebuild when full verification also fails. Its default compact model projection also omits the absolute repository root and last-indexed timestamp. Verified-state and verifier-owned failure publication use compare-and-set so concurrent invalidation or a newer verification wins. Durable live-index patches reuse incremental verification to advance the current Version's digest after a successful write, so watcher, checkpoint, indexed file-write, search-edit, and symbol-edit mutations remain eligible for incremental refresh. Patches without a verified baseline still invalidate fail-closed, and sync import/pull invalidates before its first graph write so same-version artifact mutations cannot leave stale health marked verified.

### Changed

- **Continuation contracts**: Skeleton continuations now preserve the original target and arguments while advancing only `skeletonOffset`. Workflow continuation pages use the same model projection as their first page, while internal `$N` references retain raw step data; `workflowContinuationGet` documents item-versus-character paging and its `1000` limit.
- **Runtime output contracts**: Persisted runtime artifacts remain byte-faithful after configured redaction. Default model excerpts remove only leading command-prompt echoes and recognized Node test-duration suffixes; `view: "raw"` exposes the unprojected, already-redacted excerpt.
- **Catalog paging**: `sdl.action.search` documents `maxTokens` as a bounded catalog-page control and uses `offset` for subsequent pages.
- **Edit and semantic contracts**: `file.write` retains its default `.bak` sibling, while `symbol.edit` and `search.edit` remove temporary rollback copies after success. Semantic enrichment now labels measured scores as the operational composite used by the system and omits score and basis when unavailable.

### Fixed

- **Post-remediation SDL tool QA**: Context responses now drop zero-match hot paths, summarize evidence by stable references instead of duplicating bodies, and explain answer-first fallbacks with a next action. JSON response artifacts return same-handle path recovery; runtime denials recommend compatible Windows shell commands; no-result symbol suggestions avoid repeating the failed query; bounded block windows center on requested identifiers outside the initial range; and compact semantic status distinguishes unmeasured precision while reporting bounded diagnostic severity counts. Stable `sdl.action.search` and `sdl.manual` registrations now advertise output schemas.
