...
- **GPU-aware Jina default**: Omitted/`default` Jina now selects FP16 for DirectML-first throughput sessions and quantized for CPU or deterministic sessions; only configured adjacent `dml,cpu` enables quantized CPU fallback, while explicit variants stay authoritative. npm postinstall caches and SHA-256 verifies both pinned Jina graphs, and effective-artifact changes invalidate Jina Symbol and FileSummary hashes on the next explicitly initiated semantic index.
...
- **LadybugDB close lifecycle fence**: A close request now rejects queued and later root database work while allowing active nested operations to drain, retains failed native handles for strict-close retry, caps the operation queue at 256 waiters, and requires an awaited strict close before switching database paths.
...
- **Transitive dependency advisories**: Updated the `brace-expansion`, `fast-uri`, `hono`, and `ip-address` overrides to their patched releases.
- **Saved-file symbol card freshness**: Saved-file reconciliation and deferred derived refreshes now invalidate version-keyed symbol cards, while repository generations reject stale in-flight cache publications after either update.
...
- **Semantic test-case facets**: Symbol cards can now expose optional normalized test-case metadata for JavaScript/TypeScript `node:test`, Jest, and Vitest cases plus Python pytest and unittest cases. Existing symbol search and context evidence use the facets without adding a test-only retrieval lane.
...
- **Persisted graph integrity gate**: Added revision-safe verification that keeps saved-file graph commits fast while an application-owned worker verifies the persisted LadybugDB graph independently.
  ...
  - **Fast saved-file commits**: Watcher, checkpoint, indexed `file.write`, `search.edit`, and `symbol.edit` paths validate the affected file, then commit graph rows, manifest changes, and one monotonic revision in the same transaction. They notify the verifier after commit and return without a foreground full-graph capture or synchronous fallback.
  - **Background snapshots and publication**: One coalescing verifier per repository reads the expected manifest and actual graph from one exclusive read-only snapshot. A Version-and-revision compare-and-set publishes success or failure only after the snapshot closes, so a newer edit always wins over stale verifier output.
  - **Incremental liveness**: Incremental plans combine repository-owned fileless liveness counts, affected-file buckets, and exact current pass-2 source deltas. This keeps symbol promotion, import re-resolution, call cleanup, and placeholder pruning aligned without retaining a second edge graph or trusting cleanup-query output; empty and unsupported files publish their empty expectation before persistence.
  - **Repository-safe cleanup**: Repository membership follows `SYMBOL_IN_REPO`. Cleanup removes only stale target-repository membership from shared placeholders, preserves other repositories' relationships and physical nodes, and deletes a node only when it has no memberships and is globally isolated; the delete-safety calculation conservatively includes symbols outside or shared with the target repository.
  - **Provider commit boundary**: Provider expectations begin only when the active graph commits. Auto mode no longer falls back after that boundary, where a fallback could otherwise leave partial writes.
  - **Unchanged-scan reuse**: Unchanged incremental scans verify the existing digest and reuse the latest version instead of manufacturing a new snapshot.
  - **State, availability, and recovery**: `DerivedState` records `unknown`, `verifying`, `verified`, or `failed` together with the current and verified revisions. A current manifest keeps graph reads available while verification runs or after a failed latest attempt, but `sdl.repo.status` does not claim that the latest revision is verified. Missing manifests, migrated state, and unsupported direct mutations direct operators to a full refresh; startup requeues persisted pending revisions, and destructive lifecycle operations cancel and await active verifiers before closing or deleting graph state.
...
- **Schema v24 rebuild boundary**: Schema version 24 stores semantic test-case metadata, but its nullable migration cannot backfill synthetic test symbols. Stop SDL-MCP and run `sdl-mcp index --force --safe-rebuild <absolute-new-path>` with a new absolute candidate path after upgrading.
- **Breaking `sdl.context` v2 contract**: Replaced the nested mode-driven request and synthesized answer envelope with a strict flat request, required `budget.maxTokens`, task profiles, one coverage-aware retrieval pipeline, deterministic `evidence` and `edges`, bounded omission metadata, and generic response-artifact wrapping. Removed the legacy planner, executor, context continuation, answer-first projection, and semantic retrieval-mode switch. ETags now identify the complete canonical payload before refs or wire-format wrappers.
- **Stable tool contracts**: Default `sdl.repo.status` responses separate stable repository and graph state from opt-in `includeTelemetry` operational data. Byte-stability checks use inline context with `refsMode: "off"`; response-artifact handles and session refs remain deliberately session-scoped.
- **Tool discovery and schemas**: The memory-disabled default surface exposes 34 active tools, each with an object-root MCP `outputSchema`. All seven Code Mode registrations now advertise object-root output schemas as well, and the four multi-operation gateways expose stable outer result keys instead of fieldless envelopes. `sdl.info` provides runtime and capability discovery; optional memory tools remain outside that default surface.
- **Continuation contracts**: Skeleton continuations preserve the original target and arguments while advancing only `skeletonOffset`. Workflow continuation pages use the same model projection as their first page, omit volatile `generatedAt` metadata, and keep raw step data only for internal `$N` references; `workflowContinuationGet` documents item-versus-character paging and its `1000` limit.
...
- **SDL tool QA reliability**: Windows PowerShell runtimes preserve the parent `PATHEXT` needed by nested native commands; path-redacted `sdl.info` responses use a process-stable logging placeholder; and missing natural-symbol references return at most three suggestions scoring at least `0.35`.
...
- **Non-blocking background reconciliation**: Derived graph refreshes now stay inside the normal dispatch budget instead of borrowing the destructive-index gate, so unrelated reads and rapid edits can continue while per-repository locks and LadybugDB's global writer serialization protect mutations. Completion uses a target-version CAS so stale workers cannot clear newer dirty state. Long-lived entrypoints fail closed until underlying refresh work, indexing-gate work, and queued foreground dispatch have drained before closing the database.
...
- **Post-remediation SDL tool QA**: JSON response artifacts return same-handle path recovery; runtime denials recommend compatible Windows shell commands; no-result symbol suggestions avoid repeating the failed query; bounded block windows center on requested identifiers outside the initial range; and compact semantic status reports unavailable or operational-composite score semantics with bounded diagnostic severity counts. Stable flat tools in the repository, indexing, symbol, slice, code, and policy families advertise output schemas, as do stable `sdl.action.search` and `sdl.manual` registrations.
- **SDL tool QA follow-up**: Explicit slice entries remain authoritative roots while graph traversal expands relationships, and disconnected slices return deterministic recovery guidance. Invalid response paths retain sorted valid keys and same-handle retry calls. TypeScript SCIP metadata now preserves interface kinds and declaration-wide multiline variable ranges across cards and code/edit windows.
...
- **Agent-facing tool contracts**: Default `sdl.context` output no longer embeds action durations or evidence timestamps. Public manual, action-search, retrieve, and JSON response-handle failures now use consistent actionable error envelopes; response metadata is stable and recovery calls are copyable. Missing repository roots suppress impossible refresh advice, while bounded block windows honor requested identifiers and hot-path guidance distinguishes AST identifiers from keywords.
