...
- **Context retrieval relevance**: `sdl.context` calls with `options.semantic: true` now retain each resolved named concept/action through semantic-first deduplication and final caps, prefer complete multi-term lexical matches, balance inferred-path hints against retrieval ranking, and downrank generic modules without task/path affinity. Precise evidence keeps a hard three-card per-file cap, adapts named coverage within the 20-card/eight-skeleton ceilings, and emits signature-preserving 480-character skeleton excerpts. Broad review retrieval derives evidence scope from multi-term declaration coverage, keeps inferred scope separate from caller focus, and applies bounded fan-in, fan-out, K-core, test-reference, and PageRank importance only inside the final deterministic selector. The unchanged 26-case semantic benchmark still enforces `>=85%` expected-symbol recall, `<=10%` configured noise, and zero failures.
- **Windows FTS database reopen regression**: Clean Windows child processes now create a populated FTS database and reopen it twice through the production database path without ambient OpenSSL DLL directories. The verified runtime preload remains scoped to explicit `LOAD EXTENSION fts`; dynamic package import and `Database` construction remain outside that optional capability boundary.
