...
- **Provider-first fallback pass-1 batching**: Same-run provider-first legacy fallback remains on inline TypeScript parsing while native Rust parsing and parser workers stay disabled for this mixed provider/fallback path, but fallback pass-1 graph writes now use the duplicate-key-safe fresh-copy batch accumulator. This shifts large LLVM-shaped fallback write cost out of per-file parser work and into the visible `pass1Drain` phase while preserving generic repair for unsafe relationship endpoints.
