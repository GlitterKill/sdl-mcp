...
- **Breaking**: Renamed `sdl.agent.orchestrate` → `sdl.agent.context` and `sdl.chain` → `sdl.workflow`. Existing CLAUDE.md/AGENTS.md files generated by prior `init` runs should be regenerated.
- Renamed internal modules: `src/agent/orchestrator.ts` → `context-engine.ts` (`ContextEngine` class replaces `Orchestrator`), `src/code-mode/chain-*.ts` → `workflow-*.ts`, `src/mcp/tools/agent.ts` → `context.ts`.
- Updated all MCP client templates, documentation, and tests to use new tool names.
