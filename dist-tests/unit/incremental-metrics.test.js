...
import { upsertFile, upsertSymbolTransaction, createEdgeTransaction, getMetrics, getSymbolsByRepo, getFileByRepoPath, } from "../../dist/db/queries.js";
...
describe("Incremental Metrics Calculation (PERF-L2.2)", () => {
    ...
    describe("Full metrics update", () => {
        it("should update all symbols without changedFileIds", async () => {
            ...
            assert.strictEqual(metrics1?.fan_out, 1, "symbol1 should have fan_out=1 (calls symbol2)");
            assert.strictEqual(metrics1?.fan_in, 1, "symbol1 should have fan_in=1 (called by symbol3)");
            assert.strictEqual(metrics2?.fan_in, 1, "symbol2 should have fan_in=1 (called by symbol1)");
            assert.strictEqual(metrics3?.fan_out, 1, "symbol3 should have fan_out=1 (imports symbol1)");
        });
    });
    describe("Incremental metrics update", () => {
        it("should only update affected symbols for changed files", async () => {
            ...
            assert.ok(newMetric1?.updated_at !== oldMetric1?.updated_at, "symbol1 (in changed file) should be updated");
            assert.ok(newMetric2?.updated_at !== oldMetric2?.updated_at, "symbol2 (in changed file) should be updated");
            assert.ok(newMetric3?.updated_at !== oldMetric3?.updated_at, "symbol3 (edge neighbor) should be updated");
            assert.strictEqual(newMetric4?.updated_at, oldMetric4?.updated_at, "symbol4 (unrelated) should NOT be updated");
        });
        it("should track edge neighbors correctly", async () => {
            ...
            const newMetric3 = getMetrics("symbol3");
            const newMetric4 = getMetrics("symbol4");
            assert.ok(newMetric3?.updated_at !== oldMetric3?.updated_at, "symbol3 (in changed file) should be updated");
            assert.ok(newMetric1?.updated_at !== oldMetric1?.updated_at, "symbol1 (edge neighbor - symbol3 imports symbol1) should be updated");
            assert.strictEqual(newMetric4?.updated_at, oldMetric4?.updated_at, "symbol4 (unrelated) should NOT be updated");
        });
        it("should handle multiple changed files", async () => {
            ...
            assert.ok(newMetric1?.updated_at !== oldMetric1?.updated_at, "symbol1 (in changed file) should be updated");
            assert.ok(newMetric2?.updated_at !== oldMetric2?.updated_at, "symbol2 (in changed file) should be updated");
            assert.ok(newMetric3?.updated_at !== oldMetric3?.updated_at, "symbol3 (in changed file and edge neighbor) should be updated");
            assert.strictEqual(newMetric4?.updated_at, oldMetric4?.updated_at, "symbol4 (unrelated) should NOT be updated");
        });
    });
    describe("Changed files parameter", () => {
        ...
        it("should not update anything with empty changedFileIds", async () => {
            ...
            assert.strictEqual(getMetrics("symbol1")?.updated_at, oldMetric1?.updated_at, "Should not update with empty changedFileIds");
            assert.strictEqual(getMetrics("symbol2")?.updated_at, oldMetric2?.updated_at, "Should not update with empty changedFileIds");
        });
    });
});
//# sourceMappingURL=incremental-metrics.test.js.map