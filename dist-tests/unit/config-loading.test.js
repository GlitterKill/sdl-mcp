...
describe("Config Loading (RR-H.8)", () => {
    it("should resolve default path relative to module (RR-H.8.1)", () => {
        ...
        const absoluteExpected = resolve(__dirname, "../../config/sdlmcp.config.json");
        assert.strictEqual(expectedPath, absoluteExpected, "Default config path should be resolved relative to loadConfig module");
    });
    it("should load example config successfully", () => {
        const exampleConfigPath = resolve(__dirname, "../../config/sdlmcp.config.example.json");
    ...
    it("should throw error for non-existent config", () => {
        assert.throws(() => loadConfig("/non/existent/path.json"), (err) => err.message.includes("Config file not found"));
    });
});
describe("AsyncFs Factory Pattern (RR-H.8.2)", () => {
    ...
    it("should use default config when none provided", () => {
        ...
        assert.notStrictEqual(ops1, ops2, "Factory should always create new instances");
    });
});
