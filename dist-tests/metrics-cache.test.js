...
describe("collectTestRefs Caching", () => {
    ...
    describe("Basic functionality", () => {
        it("should collect test references on first run", () => {
            writeFileSync(join(testRepoRoot, "tests", "test1.test.ts"), `
...
`);
    ...
    describe("Cache invalidation", () => {
        it("should use cached results for unchanged files", () => {
            writeFileSync(join(testRepoRoot, "tests", "cache-test.test.ts"), `calculateFanMetrics([], new Set());`);
            const result1 = collectTestRefs(testRepoRoot, symbols, config);
            const result2 = collectTestRefs(testRepoRoot, symbols, config);
            assert.deepStrictEqual(Array.from(result1.get("sym1") || []), Array.from(result2.get("sym1") || []));
        });
        ...
        it("should handle new files added between runs", () => {
            writeFileSync(join(testRepoRoot, "tests", "existing.test.ts"), `calculateFanMetrics([], new Set());`);
            const result1 = collectTestRefs(testRepoRoot, symbols, config);
            assert(result1.get("sym1")?.has("tests/existing.test.ts"));
            writeFileSync(join(testRepoRoot, "tests", "new.test.ts"), `updateMetricsForRepo("repo");`);
            const result2 = collectTestRefs(testRepoRoot, symbols, config);
    ...
    describe("Cache clearing", () => {
        it("should clear cache for specific repo", () => {
            writeFileSync(join(testRepoRoot, "tests", "clear-specific.test.ts"), `calculateFanMetrics([], new Set());`);
    ...
    describe("Multiple symbols per file", () => {
        it("should track all symbols referenced in a file", () => {
            writeFileSync(join(testRepoRoot, "tests", "multiple-symbols.test.ts"), `
calculateFanMetrics([], new Set());
...
`);
    ...
    describe("File pattern matching", () => {
        it("should match *.test.ts files", () => {
            ...
            writeFileSync(join(testRepoRoot, "src", "component.test.ts"), `calculateFanMetrics([], new Set());`);
            const result = collectTestRefs(testRepoRoot, symbols, config);
            assert(result.get("sym1")?.has("src/component.test.ts"));
        });
        it("should match *.spec.ts files", () => {
            mkdirSync(join(testRepoRoot, "src"), { recursive: true });
            writeFileSync(join(testRepoRoot, "src", "component.spec.ts"), `updateMetricsForRepo("repo");`);
            const result = collectTestRefs(testRepoRoot, symbols, config);
    ...
    describe("Error handling", () => {
        it("should handle file read errors gracefully", () => {
            writeFileSync(join(testRepoRoot, "tests", "readable.test.ts"), `calculateFanMetrics([], new Set());`);
            const result = collectTestRefs(testRepoRoot, symbols, config);
