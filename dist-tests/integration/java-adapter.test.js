...
describe("Java Adapter Tests (ML-C3.x)", () => {
    ...
    describe("ML-C3.1: Symbol Extraction", () => {
        it("should extract all symbols correctly", () => {
            ...
            assert.strictEqual(symbols.length, golden.length, "Should extract same number of symbols");
            for (let i = 0; i < symbols.length; i++) {
                const actual = symbols[i];
                const expected = golden[i];
                assert.strictEqual(actual.name, expected.name, `Symbol ${i}: name mismatch`);
                assert.strictEqual(actual.kind, expected.kind, `Symbol ${i}: kind mismatch`);
                assert.strictEqual(actual.exported, expected.exported, `Symbol ${i}: exported mismatch`);
                assert.strictEqual(actual.visibility, expected.visibility, `Symbol ${i}: visibility mismatch`);
                if (expected.signature) {
                    assert.ok(actual.signature, `Symbol ${i}: should have signature`);
                    assert.strictEqual(actual.signature?.params.length, expected.signature.params.length, `Symbol ${i}: params count mismatch`);
                    assert.strictEqual(actual.signature?.returns, expected.signature.returns, `Symbol ${i}: returns mismatch`);
                }
                assert.deepStrictEqual(actual.range, expected.range, `Symbol ${i}: range mismatch`);
            }
        ...
        it("should identify constructors", () => {
            ...
            assert.ok(tree);
            const symbols = adapter.extractSymbols(tree, content, filePath);
            const constructor = symbols.find((s) => s.name === "Symbols" && s.kind === "constructor");
    ...
    describe("ML-C3.2: Import Extraction", () => {
        ...
        it("should identify static imports", () => {
            ...
            assert.ok(tree);
            const imports = adapter.extractImports(tree, content, filePath);
            const staticImport = imports.find((i) => i.specifier === "java.util.Collections.EMPTY_LIST");
    ...
    describe("ML-C3.3: Call Extraction", () => {
        it("should extract all calls correctly", () => {
            ...
            const golden = JSON.parse(readFileSync(goldenPath, "utf-8"));
            assert.strictEqual(calls.length, golden.length, "Should extract same number of calls");
            for (let i = 0; i < calls.length; i++) {
                const actual = calls[i];
                const expected = golden[i];
                assert.strictEqual(actual.calleeIdentifier, expected.calleeIdentifier, `Call ${i}: calleeIdentifier mismatch`);
                assert.strictEqual(actual.isResolved, expected.isResolved, `Call ${i}: isResolved mismatch`);
                assert.strictEqual(actual.callType, expected.callType, `Call ${i}: callType mismatch`);
                if (expected.calleeSymbolId) {
                    assert.ok(actual.calleeSymbolId, `Call ${i}: should have calleeSymbolId`);
                }
                assert.deepStrictEqual(actual.range, expected.range, `Call ${i}: range mismatch`);
            }
        ...
        it("should create constructor edges for new expressions", () => {
            ...
            const symbols = adapter.extractSymbols(tree, content, filePath);
            const calls = adapter.extractCalls(tree, content, filePath, symbols);
            const newCall = calls.find((c) => c.calleeIdentifier.includes("new ArrayList"));
        ...
        it("should capture chained calls", () => {
            ...
            const symbols = adapter.extractSymbols(tree, content, filePath);
            const calls = adapter.extractCalls(tree, content, filePath, symbols);
            const toUpperCaseCall = calls.find((c) => c.calleeIdentifier === '"hello".toUpperCase');
        ...
        it("should identify super calls", () => {
            ...
            const symbols = adapter.extractSymbols(tree, content, filePath);
            const calls = adapter.extractCalls(tree, content, filePath, symbols);
            const superCall = calls.find((c) => c.calleeIdentifier === "super.parentMethod");
        ...
        it("should resolve calls to symbols in same file", () => {
            ...
            const symbols = adapter.extractSymbols(tree, content, filePath);
            const calls = adapter.extractCalls(tree, content, filePath, symbols);
            const nestedMethodCall = calls.find((c) => c.calleeIdentifier === "nestedMethod");
            assert.ok(nestedMethodCall, "Should extract nestedMethod call");
            assert.strictEqual(nestedMethodCall.isResolved, true);
            assert.ok(nestedMethodCall.calleeSymbolId?.endsWith("nestedMethod:method"), "Should resolve to nestedMethod symbol");
        });
    });
});
//# sourceMappingURL=java-adapter.test.js.map