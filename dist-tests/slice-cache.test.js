...
import { getSliceCacheKey, getCachedSlice, setCachedSlice, clearSliceCache, getSliceCacheStats, } from "../src/graph/sliceCache.js";
test("Slice Cache - Cache key generation", async (t) => {
    await t.test("should generate consistent cache keys for identical requests", () => {
        const request1 = {
            repoId: "repo1",
            versionId: "v1",
            taskText: "test",
        ...
        assert.strictEqual(key1, key2, "Keys should be identical regardless of array order");
    });
    await t.test("should generate different cache keys for different requests", () => {
        const request1 = {
            repoId: "repo1",
            versionId: "v1",
            taskText: "test",
        ...
        const key1 = getSliceCacheKey(request1);
        const key2 = getSliceCacheKey(request2);
        assert.notStrictEqual(key1, key2, "Keys should differ for different requests");
    });
});
test("Slice Cache - Cache operations", async (t) => {
    ...
    await t.test("should store and retrieve slices", () => {
        ...
        assert.deepStrictEqual(retrieved, mockSlice, "Should retrieve the stored slice");
    });
    await t.test("should store and retrieve slices", () => {
        const key = "test-key-1";
        setCachedSlice(key, mockSlice);
...
//# sourceMappingURL=slice-cache.test.js.map