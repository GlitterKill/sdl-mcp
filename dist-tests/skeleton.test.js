...
import { parseFile, extractSkeletonFromNode, trimSkeletonToBounds, } from "../dist/code/skeleton.js";
  ...
  High = 3,
    ...
    describe("skeleton file parsing", () => {
        ...
        it("should handle parse errors gracefully", () => {
            const content = `export function test( { return 42; }`;
            const tree = parseFile(content, ".ts");
            assert.ok(tree === null || tree.rootNode.hasError, "Should handle parse errors");
        });
    ...
    describe("skeleton extraction logic", () => {
        it("should preserve export statements", () => {
            ...
            assert.ok(skeleton.includes("export"), "Should preserve export statements");
            assert.ok(skeleton.includes("interface"), "Should preserve interface declarations");
        ...
        it("should preserve control flow statements", () => {
            ...
            assert.ok(skeletonLower.includes("else"), "Should preserve else statement");
            assert.ok(skeletonLower.includes("return"), "Should preserve return statements");
        });
        it("should elide dense function bodies with // …", () => {
            ...
            assert.ok(tree);
            const skeleton = extractSkeletonFromNode(tree.rootNode, content, []);
            assert.ok(skeleton.includes("// …") || skeleton.length < content.length, "Should elide or shorten body");
        });
        ...
        it("should handle class declarations", () => {
            ...
            assert.ok(tree);
            const skeleton = extractSkeletonFromNode(tree.rootNode, content, []);
            assert.ok(skeleton.includes("class"), "Should preserve class declaration");
            assert.ok(skeleton.includes("Test"), "Should preserve class name");
            assert.ok(skeleton.includes("constructor"), "Should preserve constructor");
        });
        it("should be deterministic for same input", () => {
            ...
            assert.strictEqual(skeleton1, skeleton2, "Skeleton should be deterministic");
        });
    ...
    describe("skeleton truncation", () => {
        it("should respect maxLines limit", () => {
            ...
            assert.ok(lines.length <= 5, `Should respect maxLines: got ${lines.length}`);
    ...
    describe("acceptance criteria", () => {
        it("should be deterministic for same input (AC1)", () => {
            ...
            const skeleton1 = extractSkeletonFromNode(tree1.rootNode, content, []);
            const skeleton2 = extractSkeletonFromNode(tree2.rootNode, content, []);
            assert.strictEqual(skeleton1, skeleton2, "AC1: Skeleton should be deterministic");
        });
        it("should be cheaper than raw code (AC2)", () => {
            ...
            assert.ok(ratio <= 0.5, `AC2: Skeleton should be cheaper than raw: ${(ratio * 100).toFixed(1)}%`);
        });
        it("should preserve important identifiers when requested (AC3)", () => {
            ...
            assert.ok(skeleton.includes("fetchUrl") || skeleton.includes("processItem"), "AC3: Should include at least one important identifier");
        });
    });
});
