...
class TestHarness {
    ...
    async runToolDiscovery(profile) {
        console.log(`\n--- Tool Discovery for ${profile.name} ---`);
        try {
            const toolsResult = await this.client.request({ method: "tools/list", params: {} }, ListToolsRequestSchema);
    ...
    async runGoldenTasks() {
        ...
        for (const task of this.goldenTasks) {
            ...
            try {
                ...
                const duration = Date.now() - start;
                console.error(`❌ ${task.name} - Error: ${error instanceof Error ? error.message : String(error)}`);
                results.push({
                    taskName: task.name,
                    tool: task.tool,
                    passed: false,
    ...
    async runSyncAssertions(repoId) {
        ...
        try {
            ...
            const sliceData = sliceResponse;
            const sliceHandle = sliceData.sliceHandle;
            const ledgerVersion = sliceData.ledgerVersion;
            const lease = sliceData.lease;
            ...
            const notModified = refreshData.notModified;
            if (notModified === undefined) {
                result.errors.push("Refresh response missing notModified field");
                return result;
            }
            result.refreshIncremental = true;
            console.log("✅ Refresh returns incremental delta:", notModified ? "not modified" : "delta included");
            ...
            if (results.length > 0) {
                ...
                if (symbolId) {
                    ...
                    console.log("✅ ETag notModified response:", cardData.notModified ? "304 Not Modified" : "200 OK");
                }
                else {
            ...
            result.errors.push(`Sync assertions failed: ${error instanceof Error ? error.message : String(error)}`);
        }
        this.syncAssertions = result;
        return result;
    }
    async runGovernanceAssertions(repoId) {
        ...
        try {
            ...
            if (results.length > 0) {
                ...
                if (symbolId) {
                    ...
                    const windowData = windowResponse;
                    const auditHash = windowData.auditHash;
                    const decision = windowData.decision;
                    ...
                    if (!decision || typeof decision !== "string") {
                        ...
                        if (decision === "downgrade-to-skeleton") {
                            ...
                            console.log("✅ Ladder escalation (downgrade to skeleton) triggered");
                        }
                    }
                }
            }
            ...
            result.errors.push(`Governance assertions failed: ${error instanceof Error ? error.message : String(error)}`);
        }
        this.governanceAssertions = result;
        return result;
    }
    async runSkeletonDeterminismTests(repoId, runs = 5) {
        ...
        try {
            ...
            for (let i = 0; i < runs; i++) {
                ...
                const skeletonData = skeletonResponse;
            ...
            if (result.passed) {
                console.log(`✅ Determinism passed (${result.runConsistency}/${runs} consistent runs)`);
            }
        }
        catch (error) {
            result.errors.push(`Determinism tests failed: ${error instanceof Error ? error.message : String(error)}`);
        }
        return result;
    }
    async saveGoldenFixture(fixtureName, data) {
        const fixturesDir = join(__dirname, "../golden");
        ...
        try {
            ...
            console.error(`❌ Failed to save golden fixture: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    validateResponse(actual, expected) {
        ...
        const check = (actualNode, expectedNode) => {
            ...
            if (expectedNode === "object" &&
                typeof actualNode === "object" &&
                actualNode !== null) {
                const actualObj = actualNode;
                const expectedObj = expectedNode;
...
async function main() {
    ...
    try {
        ...
        for (const profile of CLIENT_PROFILES) {
            ...
            console.log(`Tool Discovery: ${toolDiscoveryPassed ? "✅ PASSED" : "❌ FAILED"}`);
        ...
        if (runV05Tests && testRepoId) {
            ...
            for (const profile of CLIENT_PROFILES) {
                ...
                const governanceResult = await harness.runGovernanceAssertions(testRepoId);
                const determinismResult = await harness.runSkeletonDeterminismTests(testRepoId, 3);
                ...
                console.log(`Governance: ${governanceResult.passed ? "✅ PASSED" : "❌ FAILED"}`);
                console.log(`Determinism: ${determinismResult.passed ? "✅ PASSED" : "❌ FAILED"}`);
        ...
        for (const [profileName, stats] of Object.entries(report)) {
            ...
            console.log(`  - Golden Tasks: ${stats.passed}/${stats.passed + stats.failed}`);
            console.log(`  - Duration: ${stats.duration}ms`);
            if (stats.v05Sync) {
                console.log(`  - v0.5 Sync: ${stats.v05Sync.passed ? "✅" : "❌"}`);
            }
            if (stats.v05Governance) {
                console.log(`  - v0.5 Governance: ${stats.v05Governance.passed ? "✅" : "❌"}`);
            }
            if (stats.v05Determinism) {
                console.log(`  - v0.5 Determinism: ${stats.v05Determinism.passed ? "✅" : "❌"}`);
            }
        }
        const allProfilesPassed = Object.values(report).every((s) => s.toolDiscovery && s.failed === 0);
