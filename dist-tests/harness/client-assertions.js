...
export class ClientProfileAssertions {
    expectedTools = [
        ...
        },
        {
            ...
            description: "Request access to raw code window for a symbol with gating policy",
            inputSchema: z.object({
                repoId: z.string(),
                symbolId: z.string(),
                reason: z.string(),
        ...
        },
        {
            ...
            description: "Get skeleton view of code (signatures + control flow + elided bodies)",
            inputSchema: z
                .object({
                repoId: z.string(),
    ...
    assertToolDiscovery(actualTools) {
        const result = {
            clientName: "generic",
        ...
        for (const expectedTool of this.expectedTools) {
            const found = actualToolList.find((t) => typeof t === "object" &&
                t !== null &&
                t.name === expectedTool.name);
            if (!found) {
    ...
    assertSchemaCompatibility(actualTools) {
        const result = {
            clientName: "generic",
            toolDiscoveryPassed: true,
            schemaCompatibilityPassed: true,
        ...
        for (const actualTool of actualToolList) {
            ...
            const inputSchema = actualTool.inputSchema;
            ...
            try {
                if (inputSchema) {
                    ...
                    if (!schema.type || schema.type !== "object") {
                        result.schemaErrors.push(`${toolName}: inputSchema type must be "object"`);
                        result.schemaCompatibilityPassed = false;
                    }
                    if (!schema.properties || typeof schema.properties !== "object") {
                        result.schemaErrors.push(`${toolName}: inputSchema missing properties`);
                        result.schemaCompatibilityPassed = false;
                    }
                }
            }
            catch (error) {
                result.schemaErrors.push(`${toolName}: Schema validation error - ${error instanceof Error ? error.message : String(error)}`);
                result.schemaCompatibilityPassed = false;
            }
        }
        return result;
    }
    assertClientProfile(clientName, actualTools, clientCapabilities) {
    ...
    getClientSpecificRequirements(clientName) {
        const requirements = {
            "claude-code": [
                "stdio transport",
                "tool discovery",
...
//# sourceMappingURL=client-assertions.js.map