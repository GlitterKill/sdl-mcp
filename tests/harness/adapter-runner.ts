...
class AdapterTestHarness {
  private languages: LanguageFixture[] = [];
  ...
  discoverFixtures(): LanguageFixture[] {
    const discovered: LanguageFixture[] = [];
  ...
  async runAdapterTests(language: LanguageFixture): Promise<LanguageReport> {
    console.log(`\n${"=".repeat(50)}`);
  ...
  private async loadAdapter(language: LanguageFixture): Promise<any | null> {
    try {
      ...
      console.error(
        `Failed to load adapter for ${language.languageId}:`,
    ...
    for (const fixture of symbolFixtures) {
      ...
      try {
        ...
        if (existsSync(expectedPath)) {
          ...
          console.log(
            passed ? `✅ ${testName}` : `❌ ${testName} - count mismatch`,
          );
        } else {
          results.push({
            languageId: language.languageId,
  ...
  private findFixtures(language: LanguageFixture, type: string): string[] {
  ...
  private getExpectedPath(fixture: string, type: string): string {
    const dir = dirname(fixture);
    const base = basename(fixture);
    return join(dir, `expected-${type}.${base}.json`);
  }

  private compareSymbols(actual: unknown[], expected: unknown[]): boolean {
    return actual.length === expected.length;
  }

  private compareImports(actual: unknown[], expected: unknown[]): boolean {
    return actual.length === expected.length;
  }

  private compareCalls(actual: unknown[], expected: unknown[]): boolean {
    return actual.length === expected.length;
  }
}
...
async function main(): Promise<void> {
  ...
  for (const report of reports) {
    ...
    console.log(
      `${report.languageId}: ${allPassed ? "✅ PASSED" : "❌ FAILED"}`,
    );
