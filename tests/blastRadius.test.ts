...
import type { SymbolId, EdgeRow } from "../dist/db/schema.js";
...
describe("BlastRadius Edge Cases", () => {
  ...
  describe("edge case combinations", () => {
    ...
    it("should handle graph with no edges", () => {
      ...
      assert.strictEqual(
        result.length,
        0,
        "Should return empty array for isolated symbol",
      );
    });
  });
  ...
  describe("missing symbol handling", () => {
    it("should handle missing symbol in graph gracefully (AC2)", () => {
      const changedSymbols: SymbolId[] = ["symbol-missing"];
      const graph: Graph = {
        repoId: "test-repo",
        symbols: new Map(),
        edges: [],
      ...
      assert.ok(
        result.length >= 0,
        "Should handle missing symbol without error",
      );
    });

    it("should handle mix of present and missing symbols", () => {
      const changedSymbols: SymbolId[] = ["symbol-1", "symbol-missing"];
      const graph: Graph = {
        repoId: "test-repo",
        symbols: new Map([
          [
      ...
      assert.ok(
        result.length >= 0,
        "Should handle mix of present and missing symbols",
      );
    });
  });
});
