...
describe("Symbol References (Inverted Index)", () => {
  ...
  before(() => {
    ...
    `);
    const file1 = db.prepare("SELECT file_id FROM files WHERE repo_id = ? AND rel_path = ?").get(repoId, 'test/file.test.ts') as { file_id: number };
    ...
    `);
    const file2 = db.prepare("SELECT file_id FROM files WHERE repo_id = ? AND rel_path = ?").get(repoId, 'test/other.test.ts') as { file_id: number };
