...
describe("negative savings accounting", () => {
  it("clamps SDL overhead to zero saved tokens", () => {
    ...
    assert.equal(snapshot.totalSavedTokens, 0);
    assert.equal(snapshot.overallSavingsPercent, 0);
    assert.equal(snapshot.toolBreakdown[0]?.savedTokens, 0);
  });
});
