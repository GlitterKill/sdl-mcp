...
describe("IE-K.2: Path normalization in edge cleanup", () => {
  ...
  it("should handle relative paths with .. segments", () => {
    ...
    assert.ok(
      variants.some((v) => v.endsWith("server.ts")),
      "Should have .ts extension variant",
    );
    assert.ok(
      variants.some((v) => v.endsWith("server.js")),
      "Should have .js extension variant",
    );
  });
  ...
  it("should handle relative paths with multiple .. segments", () => {
    ...
    assert.ok(
      variants.some((v) => v.includes("db/queries")),
      "Should resolve to db/queries",
    );
    assert.ok(
      variants.some((v) => v.endsWith(".ts")),
      "Should include .ts variant",
    );
  });
  ...
  it("should try .tsx and .jsx extensions", () => {
    ...
    assert.ok(
      variants.some((v) => v.endsWith(".tsx")),
      "Should try .tsx extension",
    );
  });
  ...
  it("should handle paths without extensions", () => {
    ...
    assert.ok(
      variants.some((v) => v.endsWith("utils/helpers.ts")),
      "Should add .ts extension",
    );
    assert.ok(
      variants.some((v) => v.endsWith("utils/helpers.js")),
      "Should add .js extension",
    );
  });
  ...
  it("should handle index file resolution", () => {
    ...
    assert.ok(
      variants.some((v) => v.endsWith("config/index.ts")),
      "Should try config/index.ts",
    );
    assert.ok(
      variants.some((v) => v.endsWith("config/index.js")),
      "Should try config/index.js",
    );
  });
