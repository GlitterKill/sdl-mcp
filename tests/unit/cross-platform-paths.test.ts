...
describe("Cross-Platform Path Utilities", () => {
  ...
  describe("getRelativePath", () => {
    ...
    it("should handle same directory", () => {
      ...
      assert.ok(
        result === "." || result === "",
        "Same directory should return '.' or empty string",
      );
    });
  ...
  describe("Real-world Path Scenarios", () => {
    ...
    it("should handle deeply nested paths", () => {
      const path =
        "src\\indexer\\adapter\\treesitter\\extractors\\symbol-extractor.js";
      const normalized = normalizePath(path);
      assert.strictEqual(
        normalized,
        "src/indexer/adapter/treesitter/extractors/symbol-extractor.js",
      );
    });
  ...
  describe("Cross-Platform Consistency", () => {
    it("should produce same normalized path from different OS inputs", () => {
      ...
      assert.strictEqual(
        windowsNormalized,
        unixNormalized,
        "Normalization should be OS-agnostic",
      );
    });
    ...
    it("should produce same relative path regardless of input format", () => {
      ...
      assert.strictEqual(
        result1,
        result2,
        "Relative path calculation should be OS-agnostic",
      );
    });
