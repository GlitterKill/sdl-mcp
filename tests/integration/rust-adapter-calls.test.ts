...
`;
      ...
      const simpleFunctionCalls = calls.filter((c) =>
        c.calleeIdentifier.includes("simple_function"),
      );

      assert.ok(
        simpleFunctionCalls.length >= 2,
        `Should extract at least 2 calls to simple_function, got ${simpleFunctionCalls.length}`,
      );
...
`;
      ...
      assert.ok(
        ...
        `Should extract method calls, got ${methodCalls.length}`,
      );
...
`;
      ...
      assert.ok(
        newCall.calleeIdentifier.includes("::"),
...
`;
      ...
      assert.ok(
        macroCalls.length >= 2,
        `Should extract at least 2 macro calls, got ${macroCalls.length}`,
      );

      for (const macroCall of macroCalls) {
        assert.strictEqual(
          macroCall.isResolved,
          false,
          "Macro calls should be unresolved",
        );
        assert.strictEqual(
          macroCall.callType,
          "dynamic",
          "Macro calls should have dynamic type",
        );
      }
    });
    ...
    it("should extract calls from comprehensive fixture", () => {
      ...
      console.log(
        `✓ Extracted ${symbols.length} symbols and ${calls.length} calls`,
      );
...
`;
      ...
      assert.ok(
        pushCalls.length >= 2,
        `Should extract at least 2 push calls (chained), got ${pushCalls.length}`,
      );
      ...
      for (const call of pushCalls) {
        assert.strictEqual(
          call.callType,
          "method",
          "Chained push calls should be method type",
        );
      }
    });
...
`;
      ...
      assert.strictEqual(
        outerCall.callerNodeId,
        globalFunc.nodeId,
        "Call should be attributed to global function",
      );
    });
  });
});
