...
For behavior-mode records, `artifacts.promptPath` points at the rendered prompt, `artifacts.agent` contains the command, exit code, stdout, and stderr, and `artifacts.changedFiles` lists files changed by the agent command. SDL behavior prompts expose the live MCP server and generic SDL usage guidance, but they do not include task-specific retrieved HTTP context. Codex records with session counts include `artifacts.codexSession` and `artifacts.codexSterility`; the prompt-tokenizer fallback is retained as `artifacts.estimatedTokens`. Codex behavior runs fail before writing a record if the matching Codex session is missing token-count totals or contains Ponytail, generic plugin/app/skill instructions, or memory context.

For SDL-variant records, `artifacts.sdl` contains HTTP setup evidence: `transport`, `repoId`, `durationMs`, `index`, `retrieval`, and `context`. `transport` is `"http"`; `index` is the `/reindex-stream` completion payload; `retrieval` is intentionally empty for the default honest setup because SDLBench does not pre-run task-specific symbol searches. Actual SDL discovery should appear in behavior-mode session/tool-call evidence rather than in the setup artifact. Missing or empty HTTP indexing fails the run instead of writing estimator-backed savings.
