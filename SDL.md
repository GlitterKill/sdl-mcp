...
4. Pass through the full session token meters from `usageStats` to the user as-is, including per-tool token savings. Do not replace the token meter bars with a text summary.
