...
Codex enforcement generated by
`sdl-mcp init --client codex --enforce-agent-tools` uses a broad
`PreToolUse` matcher with an internal PID-file gate. When the SDL-MCP PID file
is absent, native tools are allowed. When it is present, repo-targeting native
source reads/searches/edits, non-SDL MCP file/search tools, and repo-local
build/test/lint shell commands are denied so agents stay on SDL-MCP.
