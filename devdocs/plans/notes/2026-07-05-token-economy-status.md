...
## Backlog Resolution Verification (2026-07-10)

- [x] Integrated shared gates passed before the external smoke: build, typecheck, focused track tests, lint (0 errors, 48 existing warnings), documentation/config/schema checks, determinism (4/4), golden validation (20/20), and the corrected full suite (611/611 files, exit 0).

- [ ] External benchmark proof failed at the preserved isolated-artifact boundary. Command: `npm run benchmark:external -- --repo-id scip-io --out-dir .benchmark/external/scip-io-cold-smoke-v1 --cache-mode cold --repeats 1`; exit code: 1; artifact: `.benchmark/external/scip-io-cold-smoke-v1`; manifest SHA-256: `7f90cbbcc26e190fe40dcde44a13e35c09e12446465ee1733c01f9adb005bb13`; stable failure boundary: `raw-result-missing`. The child benchmark exited 0, resolved `inputs/baseline.json`, `inputs/threshold.json`, and `raw/repeat-001.benchmark.json` against the worktree instead of the artifact root, skipped threshold evaluation, and wrote `.benchmark/latest.json` (SHA-256 `b00f9f709a2b5f13d2d5ec4000d2b56d4c9105daae3f0fc7996167f6ed0966be`). The source and staged threshold hashes remained identical at `ff24597f2609084eead90dfd016a07f2b132848aa715909e638c9151eb9a8cc3`. Default-DB before/after fingerprints were byte-identical at `4a33a056942d1913936db26612a382ae4fd1c704477488e019c6b81fd8fd57c6`; the corrupted default graph was not opened or modified. Next action: make the launcher resolve artifact-relative baseline, threshold, and output paths correctly (or set the child working directory to the artifact root), add a different-working-directory regression, then authorize one new cold smoke under a new stable artifact name. Preserve and never reuse or delete the `scip-io-cold-smoke-v1` evidence.
