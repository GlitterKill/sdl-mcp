...
## 2026-07-15 0.18.1 retirement

LadybugDB 0.16.1 still requires the per-patch Symbol FTS drop/rebuild workaround to avoid the historical active-index update/delete process crash. The workaround is intentionally retired only for the 0.18.1 upgrade path after SDL proves the replacement runtime boundary.

The 0.18.1 Windows gate now uses the official `@ladybugdb/core@0.18.1` package plus the temporary SDL OpenSSL runtime package `@sdl-mcp/ladybug-openssl-win32-x64@3.5.7-sdl.1` and the SDL native preload shim. Ladybug core is required rather than optional because SDL cannot start without its graph database. Raw `--omit=optional` installs are unsupported because Ladybug publishes its required platform binary as a transitive optional dependency; the release matrix instead proves an MCP handshake and graceful FTS unavailability with SDL's native accelerator explicitly disabled. The clean subprocess creates a fresh home, sanitizes `PATH`, verifies no ambient OpenSSL DLLs are found, preloads `libcrypto-3-x64.dll` then `libssl-3-x64.dll` from the npm package `bin` directory, runs `INSTALL fts` and `LOAD EXTENSION fts`, creates Symbol FTS, performs 25 raw update/delete/query cycles through `QUERY_FTS_INDEX`, calls real `patchSavedFile()` with the FTS index active, and shuts down cleanly. The replacement `3.5.7-sdl.2` package contract pins the actual OpenSSL 3.5.7 signer (`BA5473A2B0587B07FB27CF2D216094DFD0CB81EF`) and must be published and registry-verified before SDL changes its exact runtime dependency.

Local retirement evidence:
...
Result: all five clean subprocess runs exited 0. Each run passed `missing-runtime-baseline` and `fixed-regression`; `legacy-crash-baseline` was skipped because the active tree is on `kuzu@npm:@ladybugdb/core@0.18.1`; the no-SDL-runtime upstream probe remains skipped because upstream still requires SDL OpenSSL provisioning.

With that evidence, `withSymbolFtsPausedForPatch()` and its `dropFtsIndex()` / `ensureFtsIndexForNonEmptyTable()` wrapper were removed. `patchSavedFile()` now calls the normal mutation transaction directly, and `tests/unit/file-patcher-fts-lifecycle.test.ts` guards against reintroducing the drop/rebuild wrapper.

Temporary Windows OpenSSL provisioning remains in place until the upstream removal gate passes. The tracked upstream issue is [LadybugDB/ladybug#685](https://github.com/LadybugDB/ladybug/issues/685). The authoritative removal signal is not a version guess; it is the clean Windows no-SDL-runtime probe passing with an upstream Ladybug release and FTS extension, followed by the same repeated raw mutation and real `patchSavedFile()` gates.
