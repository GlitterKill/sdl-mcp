...
The 0.18.1 Windows gate now uses the official `@ladybugdb/core@0.18.1` package plus the temporary SDL OpenSSL runtime package `@sdl-mcp/ladybug-openssl-win32-x64@3.5.7-sdl.1` and the SDL native preload shim. The clean subprocess creates a fresh home, sanitizes `PATH`, verifies no ambient OpenSSL DLLs are found, preloads `libcrypto-3-x64.dll` then `libssl-3-x64.dll` from the npm package `bin` directory, runs `INSTALL fts` and `LOAD EXTENSION fts`, creates Symbol FTS, performs 25 raw update/delete/query cycles, calls real `patchSavedFile()` with the FTS index active, and shuts down cleanly.
