...
- Never expose or fuse a Symbol until its `Symbol.repoId` ownership matches the requested repository.
...
Query `QUERY_VECTOR_INDEX` against the physical `SymbolVectorEmbedding` table. In the same Cypher statement, join each returned embedding node to `Symbol` by `symbolId` and return only its `symbolId`, distance, and a boolean ownership marker for `Symbol.repoId = <requested repo>`. No foreign Symbol content reaches TypeScript, and rows whose ownership marker is false are discarded before ranking, evidence, fusion, or public output. Keeping the bounded raw row count lets the caller distinguish an exhausted global window from repository filtering.
...
The exact query joins `Symbol` to `SymbolVectorEmbedding`, filters by `repoId` and non-null model vector, computes `array_cosine_similarity`, and orders by score descending with `symbolId` as the deterministic tie-break. It returns only the requested top-K.
