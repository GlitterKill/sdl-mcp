...
- use `file.read` inside `sdl.chain` for non-indexed files with targeted modes (search, jsonPath, offset/limit)
...
3. confirm the agent uses `sdl.agent.orchestrate` for code context retrieval
4. confirm `sdl.chain` is used for runtime execution and multi-step operations, not context retrieval
