...
| `providerFirst.readyState` | `"graphPlusAlgorithms"` | `"graphPlusAlgorithms"` | The target first ready state requires graph data plus derived algorithms. Current SCIP provider-first materialization runs derived graph state only when source lines prove call references against the expected SCIP symbol text. Unresolved references, non-call references, stale source/SCIP mismatches, or unavailable source lines remain occurrence facts and can leave graph-derived readiness dirty with a health reason. Semantic refresh is deferred during provider-first indexing and tracked as separate semantic readiness work. |
