...
| `additionalModels`             | `string[]`                              | `["nomic-embed-text-v1.5"]`      | Extra embedding models to populate at index time so multiple vector lanes can contribute to hybrid fusion. Each entry gets a separate embedding pass during indexing. Set to `[]` to disable secondary models. Unknown model names are skipped.                                                     |
