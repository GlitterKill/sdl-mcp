...
| -------------------- | ------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pipeline`           | `"legacy" \| "providerFirst" \| "auto"` | `"auto"` | `auto` selects an executable provider-first path when one is safe, otherwise legacy remains the fallback. `providerFirst` requires an executable provider path and errors instead of silently falling back when activation, LSP, incremental, or coverage support is incomplete. `legacy` forces the existing indexer. |
| `providerFirst.activation` | `"shadowDb"` | `"shadowDb"` | Target activation model for full provider-first builds. Current provider-first SCIP execution is gated until the shadow `.lbug` loader and lock-aware activation handoff are implemented. |
| `providerFirst.readyState` | `"graphPlusAlgorithms"` | `"graphPlusAlgorithms"` | The first ready state requires graph data plus derived algorithms. Semantic summaries and embeddings are separate readiness work. |
