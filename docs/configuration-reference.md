...
| -------------------- | ------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pipeline`           | `"legacy" \| "providerFirst" \| "auto"` | `"auto"` | `auto` selects an executable provider-first path when SCIP coverage is complete, otherwise legacy remains the fallback. `providerFirst` requires an executable provider path and errors instead of silently falling back when SCIP execution, LSP, incremental, or coverage support is incomplete. `legacy` forces the existing indexer. |
