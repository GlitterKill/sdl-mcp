...
- **Pipeline selection:** `legacy` forces the existing tree-sitter/Rust path. `providerFirst` requires a safe provider run for full SCIP refreshes. `auto` uses provider-first only when a trustworthy provider path is available and otherwise falls back to legacy.
