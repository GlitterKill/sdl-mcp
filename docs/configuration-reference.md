...
| `providerFirst.readyState` | `"graphPlusAlgorithms"` | `"graphPlusAlgorithms"` | The target first ready state requires graph data plus derived algorithms. Current SCIP provider-first materialization now runs derived graph state when source text proves call references; unresolved or non-call references remain occurrence facts for later targeted fallback/pass-2 work. Semantic refresh is deferred during provider-first indexing and tracked as separate semantic readiness work. |
