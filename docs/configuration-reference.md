...
| -------------------- | ------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pipeline`           | `"legacy" \| "providerFirst" \| "auto"` | `"auto"` | `auto` selects provider-first when SCIP or configured LSP coverage exists, otherwise the legacy tree-sitter/Rust indexer remains the only path. `legacy` forces the existing indexer. |
| `providerFirst.activation` | `"shadowDb"` | `"shadowDb"` | Full provider-first builds use a shadow LadybugDB activation model so the active graph is not replaced until validation succeeds. |
