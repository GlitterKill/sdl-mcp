...
- **Pipeline selection:** `legacy` uses the existing tree-sitter/Rust path only when SCIP/LSP provider inputs are disabled. If SCIP or LSP inputs are enabled, SDL-MCP coerces the run to provider-first and reports a diagnostic. `providerFirst` requires a safe provider run for SCIP/LSP refreshes. Incremental provider-first refreshes require an existing provider-first bootstrap in the active graph; otherwise SDL-MCP uses the legacy incremental lane. Incremental SCIP provider-first refreshes also require `scip.generator.enabled` so SDL-MCP can pass changed files to `scip-io`; static SCIP index files without a generator fall back instead of treating stale output as fresh. `auto` uses provider-first only when a trustworthy provider path is available and otherwise falls back to pure legacy without SCIP/LSP overlay ingestion.
...
When local CPU embedding settings are omitted, SDL-MCP samples free memory once at startup and applies the same preset for `performanceTier: "auto"` and pinned tiers. It uses `min(8, estimated physical cores)` as the CPU width; `clamp(floor(free memory / 1 GiB), 1, 8)` as the memory width; and the smaller width for embedding concurrency. Symbol batches use `8`.

Explicit values always win. The JSON Schema values of `1` for concurrency and `32` for symbol batch size remain validation fallbacks, not these runtime defaults; FileSummary batches remain `4`.
...
| `embeddingBatchSize`            | `number`                                | auto (`32` schema fallback)        | `1-128`. When omitted, `8`. Larger batches amortise tokenizer and session bind/unbind costs, but raise peak memory. Length-bucketing keeps tokenizer pad waste bounded. Explicit values win. |
...
| `modelVariant`                  | `string`                                | automatic for Jina                 | For Jina, omitted/`"default"` is provider-aware: Windows DirectML-first throughput sessions use FP16; non-Windows automatic, CPU, and deterministic sessions use quantized. Only configured adjacent `["dml","cpu"]` on Windows adds the quantized CPU fallback candidate. Explicit non-default variants remain authoritative on every platform. See [semantic-embeddings-setup](feature-deep-dives/semantic-embeddings-setup.md). |
| `executionProviders`            | `string[]`                              | `["cpu"]`                          | ONNX Runtime providers in configured priority order. The session helper filters unsupported providers and appends `cpu`; Jina automatic variant selection uses the configured order before that normalization. DirectML forces sequential execution, disables ONNX memory patterns, and serializes `Run()` per cached session. |
...
npm postinstall caches and SHA-256 verifies both pinned Jina graphs (~321 MB FP16 and ~162 MB quantized); Nomic delivery remains the pinned quantized graph. When the effective Jina graph changes, Jina Symbol and FileSummary card hashes invalidate on the next explicitly initiated semantic index.
...
| `generator.enabled`               | `boolean`                                 | `false`     | Runs `scip-io index` before refresh. Required for provider-first SCIP incremental refreshes because SDL-MCP passes changed files through `--files-from` and writes a scoped temp output.                                                                  |
