...
| `watcherHealth`    | object \| null | File watcher state: `provider`, `configuredProvider`, `fallbackReason`, `enabled`, `running`, `filesWatched`, `eventsReceived`, `eventsProcessed`, `errors`, `queueDepth`, `restartCount`, `stale`, `lastEventAt`, `lastSuccessfulReindexAt`, and Watchman warning/recrawl/fresh-instance diagnostics when present |
...
6. **Code mode** — Inline code executes without shell quoting. Default Node snippets read ESM source from stdin so cwd-relative imports work without repo-local temp files. Runtimes that need a script file, including Node code with user `stdin`, use an OS temp file that is cleaned up after execution.
