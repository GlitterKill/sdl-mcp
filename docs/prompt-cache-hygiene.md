...
Graph-integrity status follows the same contract. `repo.status` exposes only the deterministic state, graph version, content digest, and recovery action; mismatch diagnostics and internal error text stay in operational logs. The digest uses a fixed canonical tuple of fields that remain immutable through post-index refresh plus explicit file/symbol ordering, so the same authoritative graph produces the same value across processes and unchanged re-indexes. Mutable summary provenance such as `summarySource` is intentionally excluded because semantic refresh can rewrite it before final verification.
