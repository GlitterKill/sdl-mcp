...
When a user asks for a token-savings report, or when telemetry debugging needs visible evidence, agents should copy the returned `formattedSummary` verbatim inside a fenced `text` block. Do not paraphrase, shorten, reformat, or strip the bar characters; if the stats call cannot be captured, state the reason.
