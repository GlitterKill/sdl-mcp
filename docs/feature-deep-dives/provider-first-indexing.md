...
The same verification boundary covers parser provenance. `FileParserState` records the engine, engine contract, adapter key, and language for each file parsed by SDL-MCP; provider-owned files without a replayable SDL-MCP contract intentionally have no record. `RepoParserState` binds a deterministic `complete` or `partial` membership summary to the graph version and revision. Final publication writes that summary and graph verification in one transaction; neither state advances alone. An existing durable file remains fail-closed for live parsing unless its own parser state is present, structurally valid, available, and matches the recorded contract.
