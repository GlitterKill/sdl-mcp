...
Incremental indexing starts from the previous verified digest, re-reads the active graph before any deletion, and replaces only changed or removed per-file digests with current authoritative results. When a scan finds no changed files, SDL-MCP still scans and validates the current persisted graph before recovery or versioning work; a clean result reuses the existing latest version, while an unknown, failed, or mismatched baseline requires a full refresh. An incremental mutation does not run when the latest version lacks a verified baseline. Run a full refresh after upgrading an existing database or after any integrity failure to rebuild and establish that baseline. If that full refresh also fails integrity verification, stop SDL-MCP, delete the configured `.lbug` database directory, and rebuild it from source; this clean rebuild is required when LadybugDB safely refuses to retire a large stale tail in place.
...
For repeat full indexes, active stale-symbol retirement is chunked and allowed through medium provider sets, currently up to 50k provider symbols. Medium refreshes can delete stale rows and reload provider `Symbol`/ownership/edge rows through the known-fresh `COPY` writers instead of falling onto the slow merge-safe symbol path.

For very large repeat full indexes, active stale-symbol retirement is capped because LadybugDB can spend minutes or crash while deleting or merging tens of thousands of existing provider nodes. When the cap is exceeded and provider rows already exist in the active graph, SDL-MCP skips direct active graph rewrites and reuses the existing active provider rows regardless of the current generated SCIP input fingerprint.
