...
- Omitted/`default` Jina uses quantized on CPU and in deterministic sessions, or FP16 in DirectML-first throughput sessions. Only configured adjacent `dml,cpu` adds the quantized CPU candidate; explicit variants stay authoritative. Unsupported-provider filtering and CPU appending happen later in the ONNX session layer.
