import { getParser, createQuery } from "./grammarLoader.js";
...
export function parseFile(content, extension) {
    try {
        ...
        process.stderr.write(`[sdl-mcp] Failed to parse TypeScript file (extension: ${extension}): ${error instanceof Error ? error.message : String(error)}\n`);
        return null;
    }
}
export function queryTree(tree, query) {
    try {
        ...
        const queryPreview = query.length > QUERY_PREVIEW_LENGTH
            ? query.substring(0, QUERY_PREVIEW_LENGTH) + "..."
            : query;
        process.stderr.write(`[sdl-mcp] Failed to execute tree query "${queryPreview}": ${error instanceof Error ? error.message : String(error)}\n`);
        return [];
    }
}
//# sourceMappingURL=tsTreesitter.js.map