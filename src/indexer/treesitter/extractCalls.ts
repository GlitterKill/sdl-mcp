...
export interface ExtractedCall {
  ...
  candidateCount?: number; // For ambiguous calls (36-1.3)
  range: {
    startLine: number;
    startCol: number;
    endLine: number;
...
export function extractCalls(
  ...
  for (const match of standardMatches) {
    ...
    if (calleeCapture) {
      ...
      if (objCapture) {
        ...
        if (
          calleeCapture.node.text === "require" ||
  ...
  for (const match of [
    ...
    const propCapture = match.captures.find(
      (c: QueryCapture) => c.name === "prop",
    );
    const objCapture = match.captures.find(
      (c: QueryCapture) => c.name === "obj",
    );

    if (!propCapture) continue;
  ...
  for (const match of computedMatches) {
    ...
    const indexCapture = match.captures.find(
      (c: QueryCapture) => c.name === "index",
    );
  ...
  for (const match of taggedMatches) {
    ...
    let isResolved = false;
    let calleeSymbolId: string | undefined;
    const callType: ExtractedCall["callType"] = "tagged-template";
 ...
 */
function extractCallsFromArrowBody(
  body: SyntaxNode,
  parentCallerNodeId: string,
  extractedSymbols: ExtractedSymbol[],
  symbolMap: Map<string, ExtractedSymbol>,
  calls: ExtractedCall[],
  seenCallNodes: Set<number>,
  ...
  if (body.type === "call_expression" && !seenCallNodes.has(body.id)) {
    ...
    const callInfo = extractSingleCall(
      body,
      extractedSymbols,
      symbolMap,
      parentCallerNodeId,
...
function extractSingleCall(
  ...
  if (!calleeIdentifier) return null;

  const callerNodeId =
    overrideCallerNodeId ?? findEnclosingSymbol(callNode, extractedSymbols);
