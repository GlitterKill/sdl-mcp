...
function parseImportNode(node, specifier) {
        ...
        switch (child.type) {
            ...
            case "identifier": {
                if (node.type === "export_statement" && !result.defaultImport) {
                    ...
                    if (previousChild?.type !== "named_imports" &&
    ...
    if (node.type === "import_statement") {
        const hasSource = node.children.some((c) => c.type === "from" || c.type === "source");
        if (!hasSource) {
...
function extractNamedImports(node) {
    const names = [];
    for (const child of node.children) {
