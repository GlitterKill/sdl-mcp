...
import {
  getClusterMembersWithScoresForRepo,
  getClustersForRepo,
  type ClusterMemberForRepoRow,
...
import {
  ...
  type EdgeRow,
} from "./ladybug-edges.js";
import {
  getFileSummariesForRepo,
  type FileSummaryRow,
} from "./ladybug-file-summaries.js";
import { getProcessesForRepo, getProcessStepsForRepo, type ProcessStepForRepoRow } from "./ladybug-processes.js";
import type { MetricsRow } from "./ladybug-repos.js";
import {
  getShadowClusterMembersForRepo,
  getShadowClustersForRepo,
  type ShadowClusterMemberForRepoRow,
} from "./ladybug-shadow-clusters.js";
import { getVersion, type SymbolVersionRow } from "./ladybug-versions.js";
...
interface AuxiliarySymbolQueryRow {
  symbolId: string;
  repoId: string | null;
  kind: string | null;
  name: string | null;
...
async function copyFinalizedRows(params: {
  ...
  const version = await getVersion(params.activeConn, params.versionId);
  if (!version) {
  ...
  const edges = (
    await getEdgesByRepo(params.activeConn, params.repoId)
  ).filter(
    (edge) =>
  ...
  const activeRealSymbols = await readRealSymbolsForRepo(
    params.activeConn,
    params.repoId,
    params.stagedAuxiliarySymbolIds,
  );
  const activeRealSymbolIds = new Set(
    activeRealSymbols.map((symbol) => symbol.symbolId),
  );
  ...
  const fileSummaries = await getFileSummariesForRepo(
    params.activeConn,
    params.repoId,
  );
  const clusters = await getClustersForRepo(
    params.activeConn,
    params.repoId,
  );
  const clusterMembers = await getClusterMembersWithScoresForRepo(
    params.activeConn,
    params.repoId,
  );
  const processes = await getProcessesForRepo(
    params.activeConn,
    params.repoId,
  );
  const processSteps = await getProcessStepsForRepo(
    params.activeConn,
    params.repoId,
  );
  const shadowClusters = await getShadowClustersForRepo(
    params.activeConn,
    params.repoId,
  );
  const shadowClusterMembers = await getShadowClusterMembersForRepo(
    params.activeConn,
    params.repoId,
  );
...
async function readRealSymbolsForRepo(
  conn: Connection,
  repoId: string,
  ...
  const rows = await queryAll<AuxiliarySymbolQueryRow>(
    conn,
  ...
  return rows.map((row) => symbolRowFromQuery(row, repoId, "real"));
}
...
function isEdgeRelCopySafe(row: EdgeRow): boolean {
  return (
...
function isShadowClusterMemberRelCopySafe(
  row: ShadowClusterMemberForRepoRow,
): boolean {
  return (
    copyRelEndpointIsSafe(row.symbolId) &&
...
function edgeToCopyCells(row: EdgeRow): unknown[] {
  return [
    row.fromSymbolId,
    row.toSymbolId,
    row.edgeType,
...
function pass2CppProvenanceIsCopySafe(row: EdgeRow): boolean {
...
function symbolRowFromQuery(
  ...
  return {
    ...
    rangeStartLine: toNumber(row.rangeStartLine ?? 0),
    rangeStartCol: toNumber(row.rangeStartCol ?? 0),
    rangeEndLine: toNumber(row.rangeEndLine ?? 0),
    rangeEndCol: toNumber(row.rangeEndCol ?? 0),
    astFingerprint: row.astFingerprint ?? row.symbolId,
    signatureJson: row.signatureJson,
    summary: row.summary,
    summaryQuality: toNumber(row.summaryQuality ?? 0),
    ...
    symbolStatus: row.symbolStatus ?? fallbackStatus,
...
async function ensureRelationshipEndpointRepoLinks(
  conn: Connection,
  rows: AuxiliarySymbolRow[],
): Promise<void> {
  if (rows.length === 0) return;
...
async function upsertClusterMembersFallback(
  conn: Connection,
  members: ClusterMemberForRepoRow[],
): Promise<void> {
...
async function upsertShadowClusterMembersFallback(
  conn: Connection,
  members: ShadowClusterMemberForRepoRow[],
): Promise<void> {
  if (members.length === 0) return;
  const chunkSize = 256;
  for (let i = 0; i < members.length; i += chunkSize) {
...
async function readDerivedStateRow(
  ...
  return {
    ...
    processesDirty: toBoolean(row.processesDirty),
    algorithmsDirty: toBoolean(row.algorithmsDirty),
    summariesDirty: toBoolean(row.summariesDirty),
    embeddingsDirty: toBoolean(row.embeddingsDirty),
...
async function readRealSymbolVersionsForRepoAtVersion(
  ...
  const exclusionClause =
    excluded.length > 0 ? "AND NOT s.symbolId IN $excludedSymbolIds" : "";
  return await queryAll<SymbolVersionRow>(
    conn,
