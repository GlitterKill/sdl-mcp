...
export type ClusterInstance = { repoId: string; clusterId: string; position: THREE.Vector3; symbolCount: number; object: THREE.InstancedMesh };
...
export class UniverseRenderer {
  ...
  renderRepo(repo: UniverseRepo, clusters: Cluster[], layout: LayoutResponse): void {
    ...
    clusters.forEach((cluster, index) => {
      ...
      mesh.setColorAt(index, color.set(hashColor(cluster.clusterId)));
      this.clusters.set(repo.repoId + ":" + cluster.clusterId, { repoId: repo.repoId, clusterId: cluster.clusterId, position: v.clone().add(repoGroup.position), symbolCount: cluster.memberCount, object: mesh });
    });
  ...
  pulseCluster(repoId: string, clusterId: string): void {
    ...
    setTimeout(() => cluster.object.scale.setScalar(1), 350);
  }

  applyCommunityColors(): void {
    const color = new THREE.Color();
    for (const cluster of this.clusters.values()) {
      const mesh = cluster.object;
      for (let index = 0; index < mesh.count; index += 1) mesh.setColorAt(index, color.set(hashColor(cluster.clusterId)));
      if (mesh.instanceColor) mesh.instanceColor.needsUpdate = true;
    }
  }
