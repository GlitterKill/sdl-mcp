export const SDL_MCP_SERVER_INSTRUCTIONS = [
  ...
  "5. Read and write non-indexed files through `file.read` and `file.write`; edit indexed source through `symbol.edit` or SDL file symbol-edit preview/apply.",
  "6. Run repo-local commands through `runtimeExecute` inside `sdl.workflow` with `outputMode: \"minimal\"`, `persistOutput: true`, and an explicit `timeoutMs`; query logs later with `runtimeQueryOutput` and focused `queryTerms`.",
