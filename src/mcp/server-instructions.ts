export const SDL_MCP_SERVER_INSTRUCTIONS = [
  ...
  '8. Before completion, call `usageStats` with `scope: "session"` and `persist: true`; include the returned `formattedSummary` verbatim in a fenced `text` block, then report any `.bak` cleanup.',
].join("\n");
