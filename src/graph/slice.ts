...
function resolveStartNodes(
  ...
  const startNodes = new Set<SymbolId>();
  ...
  if (request.taskText) {
    ...
    for (const token of tokens) {
      const results = db.searchSymbolsLite(
        request.repoId,
        token,
        DB_QUERY_LIMIT_DEFAULT,
      );
...
function getEdgeWhy(edgeType: EdgeType, symbolName: string): string {
  switch (edgeType) {
    case "call":
...
export function estimateTokens(cards: SymbolCard[]): number {
  let total = 0;
