...
export class EventLogReplayer {
  ...
  replay(repoId: RepoId, toVersion?: VersionId): ProjectedState {
    ...
    const relevantEvents = this.events.filter((event) => {
      if (event.repoId !== repoId) return false;
      if (toVersion && event.versionId > toVersion) return false;
      return true;
    });
  ...
  validateParity(
    ...
    expectedEdges: Map<string, { from: SymbolId; to: SymbolId; type: EdgeType }>,
  ): {
    passed: boolean;
    symbolMatch: number;
    symbolTotal: number;
